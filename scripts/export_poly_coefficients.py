...
def main():
    ...
    # Write C++ data header
    out_path = Path("cpp/filament_mixer_data.inc")
    out_path.parent.mkdir(parents=True, exist_ok=True)

    with open(out_path, "w") as f:
        f.write("// Auto-generated by scripts/export_poly_coefficients.py\n")
        f.write("// Do not edit manually.\n")
        f.write(f"// Degree-{degree} polynomial, {n_features} features, {n_inputs} inputs\n\n")
        f.write(f"static const int POLY_DEGREE = {degree};\n")
        f.write(f"static const int N_FEATURES = {n_features};\n")
        f.write(f"static const int N_INPUTS = {n_inputs};\n\n")

        # Powers matrix (n_features x n_inputs) — integer exponents
        f.write(format_c_int_array("POWERS", powers))
        f.write("\n\n")

        # Coefficient matrix (n_features x 3) — transposed from sklearn's (3 x n_features)
        f.write(format_c_array("COEF", coef.T))
        f.write("\n\n")

        # Intercept (3,)
        f.write(format_c_array("INTERCEPT", intercept))
        f.write("\n")
